  ...
  switch(ui_Robot_State_Index)
  {
    ...
    case 1:    //Robot Run after 3 seconds
    {
      if(bt_3_S_Time_Up)
      {
       ...
       /*************************************************************************************/
        if(ui_Left_Line_Tracker_Data < ui_Left_Line_Tracker_Dark - 100){
          LeftB = false;
        }
        else LeftB = true;
        if(ui_Right_Line_Tracker_Data < ui_Right_Line_Tracker_Dark - 100){
          RightB = false;
        }
        else RightB = true;
        if(ui_Middle_Line_Tracker_Data < ui_Middle_Line_Tracker_Dark - 100){
          MidB = false;
        }
        else MidB = true;
        
        Serial.println("Vals")
        Serial.println(ui_Left_Line_Tracker_Data);
        Serial.println(ui_Middle_Line_Tracker_Data);
        ...
        if((!LeftB && !MidB && !RightB)||(LeftB && !MidB && RightB)){
          bt_Motors_Enabled = true;
        }
        else if((!LeftB && !MidB && RightB)||(!LeftB && MidB && RightB)){
          i_Left_Motor_Speed = ci_Left_Motor_Stop;
        }
        else if((LeftB && !MidB && !RightB)||(LeftB && MidB && !RightB)){
          ui_Right_Motor_Speed = ci_Right_Motor_Stop;
        }
        else if(LeftB && MidB && RightB){ 
